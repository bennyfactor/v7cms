function cmsApp() {
  return {
    ...
    createNewPost() {
      this.currentPost = { title: '', slug: '', content: '', workflow_state: 'draft', published: false, comments_enabled: true, tag_ids: [], tags: [] };
    ...
    createNewPage() {
      this.currentPage = { title: '', slug: '', content: '', parent_id: null, page_type: 'standard', content_source: 'children', items_limit: 10, position: 0, hero_image_url: '', workflow_state: 'draft', published: false, content_filter_tag_id: null };
    ...
    async loadTags() {
        try {
            const response = await fetch('/api/tags', { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
            const data = await response.json();
            this.tags = data.tags;
        } catch (error) {
