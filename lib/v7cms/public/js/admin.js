function cmsApp() {
  return {
    ...
    formSubmissions: [],
    submissionsPagination: { total: 0, page: 1, per_page: 25 },
    ...
    async saveMenuItem() {
      ...
      const itemData = {
        ...
        css_class: this.currentMenuItem.css_class || ''
      };
    ...
    async editForm(form) {
      try {
        ...
        if (response.ok) {
          ...
          this.formFields = (data.form.fields || []).map(f => ({
            ...f,
            options: f.options ? (typeof f.options === 'string' ? f.options : JSON.stringify(f.options)) : '',
            validation_rules: f.validation_rules ? (typeof f.validation_rules === 'string' ? f.validation_rules : JSON.stringify(f.validation_rules)) : ''
          }));
          this.editingForm = true;
          this.viewingSubmissions = false;
        }
    ...
    async saveForm() {
      ...
      try {
        const isNew = !this.currentForm.id;
        const method = isNew ? 'POST' : 'PUT';
        const url = isNew ? '/api/forms' : `/api/forms/${this.currentForm.id}`;
        const response = await fetch(url, {
          method,
          credentials: 'include',
          headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
          body: JSON.stringify({
            ...
            slug: this.currentForm.slug,
            description: this.currentForm.description,
            submit_button_text: this.currentForm.submit_button_text,
            success_message: this.currentForm.success_message,
            notification_email: this.currentForm.notification_email,
            store_submissions: this.currentForm.store_submissions,
            send_notifications: this.currentForm.send_notifications,
            require_recaptcha: this.currentForm.require_recaptcha,
            recaptcha_threshold: parseFloat(this.currentForm.recaptcha_threshold),
            spam_behavior: this.currentForm.spam_behavior,
            published: this.currentForm.published
          })
        });
        if (response.ok) {
          const data = await response.json();
    ...
    async deleteForm(form) {
      if (!confirm(`Delete form "${form.name}"? This will also delete all submissions.`)) return;
      try {
        ...
        if (this.editingForm && this.currentForm.id === form.id) {
    ...
    async saveFormField() {
      const isNew = !this.currentFormField.id;
      const method = isNew ? 'POST' : 'PUT';
      const url = isNew ? `/api/forms/${this.currentForm.id}/fields` : `/api/form-fields/${this.currentFormField.id}`;

      let options = this.currentFormField.options;
      if (options && typeof options === 'string') {
        try { options = JSON.parse(options); } catch(e) { /* keep as string, server will validate */ }
      }
      let validationRules = this.currentFormField.validation_rules;
      if (validationRules && typeof validationRules === 'string') {
        try { validationRules = JSON.parse(validationRules); } catch(e) { /* keep as string */ }
      }
      ...
      try {
        const response = await fetch(url, {
          ...
          headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
          body: JSON.stringify({
            field_type: this.currentFormField.field_type,
            name: this.currentFormField.name,
            label: this.currentFormField.label,
            placeholder: this.currentFormField.placeholder,
            help_text: this.currentFormField.help_text,
            required: this.currentFormField.required,
            options: options || null,
            validation_rules: validationRules || null,
            position: this.currentFormField.position || this.formFields.length
          })
        });
        if (response.ok) {
          const data = await response.json();
          ...
          if (isNew) {
            ...
            const idx = this.formFields.findIndex(f => f.id === field.id);
            if (idx !== -1) this.formFields[idx] = field;
          }
          this.showFieldForm = false;
          this.currentFormField = {};
          await this.fetchForms();
    ...
    async deleteFormField(field) {
      if (!confirm(`Delete field "${field.label || field.name}"?`)) return;
      try {
        await fetch(`/api/form-fields/${field.id}`, {
          method: 'DELETE',
          credentials: 'include',
          headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
    ...
    fieldTypeLabel(type) {
      const labels = {
        text: 'Text', email: 'Email', textarea: 'Text Area', select: 'Dropdown',
        checkbox: 'Checkbox', number: 'Number', tel: 'Phone', url: 'URL',
        radio: 'Radio Buttons', hidden: 'Hidden'
      };
    ...
    async viewFormSubmissions() {
      this.viewingSubmissions = true;
      this.submissionsFilter = 'all';
      this.submissionsPagination.page = 1;
      await this.fetchFormSubmissions();
    },

    async fetchFormSubmissions() {
      try {
        const params = new URLSearchParams({
          page: this.submissionsPagination.page,
          per_page: this.submissionsPagination.per_page
        });
        if (this.submissionsFilter !== 'all') params.set('filter', this.submissionsFilter);

        const response = await fetch(`/api/forms/${this.currentForm.id}/submissions?${params}`, {
          credentials: 'include',
          headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        if (response.ok) {
          const data = await response.json();
          this.formSubmissions = data.submissions || [];
          this.submissionsPagination.total = data.total;
          this.submissionsPagination.page = data.page;
          this.submissionsPagination.per_page = data.per_page;
        }
      } catch (error) {
    ...
    async deleteSubmission(submission) {
      ...
      try {
        await fetch(`/api/forms/${this.currentForm.id}/submissions/${submission.id}`, {
          method: 'DELETE',
          credentials: 'include',
          headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        this.formSubmissions = this.formSubmissions.filter(s => s.id !== submission.id);
        this.submissionsPagination.total--;
      } catch (error) {
    ...
    exportSubmissions() {
      window.open(`/api/forms/${this.currentForm.id}/submissions/export`, '_blank');
    },
