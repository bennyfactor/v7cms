function cmsApp() {
  return {
    ...
    createNewPage() {
      this.currentPage = { title: '', slug: '', content: '', parent_id: null, page_type: 'standard', content_source: 'children', items_limit: 10, position: 0, published: false };
    ...
    async loadPostLayouts() {
      try {
        const response = await fetch('/api/settings/post-layouts');
        if (!response.ok) {
          console.error('Failed to load post layouts:', response.status, response.statusText);
          return;
        }
        const data = await response.json();
        this.availablePostLayouts = data.layouts || [];
      } catch (error) {
        console.error('Error loading post layouts:', error);
      }
    },
