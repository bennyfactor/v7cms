...
function setupCommentForm() {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    try {
      // Get reCAPTCHA token (Enterprise or Standard v3)
      const recaptchaObj = typeof RECAPTCHA_ENTERPRISE !== 'undefined' && RECAPTCHA_ENTERPRISE
        ? grecaptcha.enterprise
        : grecaptcha;
      const token = await recaptchaObj.execute(RECAPTCHA_SITE_KEY, { action: 'submit_comment' });
...
function setupLoadMore() {
  ...
  loadMoreBtn.addEventListener('click', async function() {
    ...
    try {
      const response = await fetch(`/api/posts/${POST_ID}/comments?limit=${COMMENTS_LIMIT}&offset=${nextOffset}`);
      const data = await response.json();

      displayComments(data.comments);
      commentsOffset = nextOffset;
