  ...
  class Form < ActiveRecord::Base
    ...
    def validate_submission(data)
      ...
      form_fields.each do |field|
        ...
        errors.concat(validate_field_type(field, field_label, value))
        errors.concat(validate_field_rules(field, field_label, value))
      end
      errors
    end
    ...
    def validate_field_type(field, label, value)
      case field.field_type
      when 'email'
        return ["#{label} must be a valid email address"] unless value.match?(URI::MailTo::EMAIL_REGEXP)
      when 'number'
        return ["#{label} must be a number"] unless value.to_s.match?(/\A-?\d+(\.\d+)?\z/)

        validate_number_range(field, label, value)
      when 'url'
        return ["#{label} must be a valid URL"] unless value.match?(%r{\Ahttps?://\S+\z}i)
      when 'tel'
        return ["#{label} must be a valid phone number"] unless value.match?(/\A[\d\s()+.-]+\z/)
      else
        []
      end || []
    end

    def validate_number_range(field, label, value)
      errors = []
      rules = field.parsed_validation_rules
      num = value.to_f
      errors << "#{label} must be at least #{rules['min']}" if rules['min'] && num < rules['min'].to_f
      errors << "#{label} must be at most #{rules['max']}" if rules['max'] && num > rules['max'].to_f
      errors
    end
    ...
    def validate_pattern(rules, label, value)
      return [] unless rules['pattern']

      begin
        return ["#{label} format is invalid"] unless value.match?(Regexp.new(rules['pattern']))
      rescue RegexpError
