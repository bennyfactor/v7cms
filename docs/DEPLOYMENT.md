...
```bash
docker exec v7cms bundle exec rake v7cms:regenerate
```
...
docker-compose -f docker-compose.prod.yml exec web bundle exec rake v7cms:regenerate
```
...
The `fcgi` gem is included in the Gemfile with `install_if` and installs automatically on shared hosting environments. No manual Gemfile editing is needed.
...
The `.htaccess` file is auto-generated by `bundle exec rake v7cms:htaccess` and should not be hand-edited. It provides:

- Security rules (blocks access to `.rb`, `.db`, `.env`, and other sensitive files)
- Cache headers for static assets (CSS, JS, images)
- Static-first routing (serves pre-rendered HTML directly)
- FastCGI fallback for dynamic requests
